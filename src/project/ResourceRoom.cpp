...
void ResourceRoom::precompile(bytecode::ProjectAccumulator& acc)
{
    ...
    if (m_cc_room.m_source != "")
    {
        ...
        m_cc_room.m_ast = ogm_ast_parse(m_cc_room.m_source.c_str());
    ...
    for (pugi::xml_node node_tile: node_tiles.children("tile"))
    {
        ...
        {
            asset_index_t background_asset;
            const asset::AssetBackground* object_asset = dynamic_cast<asset::AssetBackground*>(acc.m_assets->get_asset(bgname_s.c_str(), background_asset));
            if (object_asset)
            {
        ...
        if (objname_s != "" && objname_s != "<undefined>")
        {
            ...
            if (object_asset)
            {
                ...
                if (code_s != "")
                {
                    ...
                    cc.m_source = code_s;
                    cc.m_ast = ogm_ast_parse(code_s.c_str());
