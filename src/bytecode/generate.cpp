...
struct EnumData {
    ...
    std::map<std::string, bool> m_ownership;

    ~EnumData()
    {
        for (const auto& pair : m_map)
        {
            if (m_ownership.at(std::get<0>(pair)))
            {
            ...
            case ogm_ast_st_exp_identifier:
                {
                    ...
                    {
                        assert(context_args.m_globals.has_id(var_name));
            ...
            case ogm_ast_st_exp_global:
                {
                    ...
                    assert(ast.m_sub[0].m_subtype == ogm_ast_st_exp_identifier);
            ...
            case ogm_ast_st_exp_possessive:
                {
                    ...
                    assert(ast.m_sub[1].m_subtype == ogm_ast_st_exp_identifier);
                    LValue lv_prop = bytecode_generate_get_lvalue<true>(out, ast.m_sub[1], context_args);

                    // convert memory space to indirect access.
                    assert(!lv_prop.m_array_access);
                    assert(lv_prop.m_memspace == memspace_instance || lv_prop.m_memspace == memspace_builtin_instance);
                    ...
                    case ogm_ast_literal_t_number:
                        {
                            ...
                            if (s[0] == '$')
                            {
                                ...
                                {
                                    write_op(out, ldi_s32);
                                    int32_t vs = static_cast<int32_t>(v);
                                    write(out, vs);
                                }
                            }
                            else
            ...
            case ogm_ast_st_exp_possessive:
                {
                    ...
                    if (ast.m_sub[0].m_subtype == ogm_ast_st_exp_identifier)
                    {
                        ...
                        auto enum_iter = enum_map.find(static_cast<const char*>(ast.m_sub[0].m_payload));
                        if (enum_iter != enum_map.end())
                        {
                            auto& value_iter = std::get<1>(*enum_iter).m_map.at(static_cast<const char*>(ast.m_sub[1].m_payload));
                            context_args.m_enum_expression = true;
                            bytecode_generate_ast(out, *value_iter, context_args);
                            break;
                        }
                    }
                }
        ...
        case ogm_ast_st_imp_var:
            {
                ...
                if (strcmp(payload->m_type, "globalvar") == 0)
                {
        ...
        case ogm_ast_st_imp_control:
            switch(ast.m_spec)
            {
                ...
                case ogm_ast_spec_control_return:
                    ...
                    if (context_args.m_retc < ast.m_sub_count)
                    {
                        throw MiscError("Wrong number of return values (encountered "
                        + std::to_string(ast.m_sub_count) + ", expected " + std::to_string(context_args.m_retc) + ")");
                    }
...
void bytecode_generate(Bytecode& out_bytecode, const DecoratedAST& in, const Library* library, ProjectAccumulator* accumulator)
{
...
namespace
{
    void bytecode_preprocess_helper(const ogm_ast_t* ast, const ogm_ast_t* parent, uint8_t& out_retc, uint8_t& out_argc, ReflectionAccumulator& in_out_reflection_accumulator)
    {
        ...
        if (ast->m_subtype == ogm_ast_st_imp_var)
        {
            ...
            if (strcmp(declaration->m_type, "globalvar") == 0)
            {
        ...
        if (ast->m_subtype == ogm_ast_st_imp_enum)
        {
            ...
            if (map.find(enum_name) == map.end())
            {
                ...
                for (size_t i = 0; i < payload->m_identifier_count; ++i)
                {
                    ...
                    {
                        enum_data.m_map[payload->m_identifier[i]] = &ast->m_sub[i];
                        enum_data.m_ownership[payload->m_identifier[i]] = false;
                    }
                    else
                    // no enum value definition
                    {
                        ...
                        enum_data.m_ownership[payload->m_identifier[i]] = true;
                    }
                }
            }
            else
